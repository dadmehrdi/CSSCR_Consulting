...
# assign values
## assign the number 2.1 to object called a
a <- 2.1
## assign another value to a
a <- 3
...
- **Trigonometry**: Functions like `sin()`, `cos()`, and `tan()` are used for trigonometric calculations. For example, `sin(pi/2)` computes the sine of π/2, resulting in `r sin(pi/2)`.
...
CSSCR_data is a small sample (0.005%) of the census data for 2022
...
ggplot(CSSCR_data, aes(x = AGE, y = INCTOT)) +
  ...
  ylim(0, 150000) +  # Setting y-axis limits
